...
namespace DapperExtensions
{
    ...
    public static class Snapshotter
    {
        ...
        public static Snapshot<T> Start<T>(T obj) where T : BaseEntity
        {
            return new Snapshot<T>(new List<T>() { obj });
        }

        public static Snapshot<T> Start<T>(IEnumerable<T> obj) where T : BaseEntity
        {
        ...
        public class Snapshot<T> where T : BaseEntity
        {
            private static Func<T, T> cloner;
            private static Func<T, T, List<Change>> differ;
            public Dictionary<string, T> MemberWiseClone = new Dictionary<string, T>();
            //private readonly List<T> trackedObject;

            /// <summary>
            /// Creates a snapshot from an object.
            /// </summary>
            /// <param name="original">The original object to snapshot.</param>
            public Snapshot(IEnumerable<T> original)
            {
                if (original is null) return;

                foreach(var item in original)
                    MemberWiseClone.Add(item.Guid(), Clone(item));
            ...
            public DynamicParameters Diff(T entity)
            {
                if (MemberWiseClone.TryGetValue(entity.Guid(), out T clone))
                    return Diff(clone, entity);
                else 
        ...
        //    /// </summary>
        //    /// <param name="original">The original object to snapshot.</param>
        //    public Snapshot(List<T> original)
        //    {
        //        if (original is null) return;
